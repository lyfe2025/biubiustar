...
class Category extends Base
{
    ...
    protected $multiFields = "status,is_nav,sort,is_home";
    ...
    protected function addData($params)
    {
        ...
        try {
            ...
            $params['model_table_name'] = $model_talbe_names[$params['mid']] ?? '';
            $params['urlname'] = preg_replace('/[^A-Za-z0-9_-]/', '', $params['urlname']);
            //是否采用模型验证
    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            $postData['urlname'] = preg_replace('/[^A-Za-z0-9_-]/', '', $postData['urlname']);
    ...
    public function multi($ids = null)
    {
        ...
        try {
            ...
            if (array_key_exists('is_home', $values)) {
                $mid = $this->model->where($this->model->getPk(), 'in', $ids)->value('mid');
                $this->model->where('lang', $this->lang)->where('mid', $mid)->update(['is_home' => 0]);
            }
            foreach ($list as $item) {
