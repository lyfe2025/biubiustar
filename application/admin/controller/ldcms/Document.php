...
class Document extends Base
{
    ...
    protected $noNeedRight = ['category', 'getTags', 'getUserLevels','setflag','checkcustomurlname'];
    ...
    public function _initialize()
    {
        ...
        $this->view->assign('flags', config('ldcms.flags'));
    ...
    public function setflag()
    {
        if ($this->request->isPost()) {
            ...
            $flagArr = array_filter(explode(',', $flag));
            if (empty($flagArr)) {
                $this->error(__('Please select flag'));
            }

            $documentList = $this->model->where('id', 'in', $ids)->select();
            $newflagArr = [];
            foreach ($documentList as $index => $item) {
                $oldFlagArr = explode(',', $item['flag']);
                if ($type == 'add') {
                    $newflagArr = array_merge($oldFlagArr, $flagArr);
                } else {
                    $newflagArr = array_diff($oldFlagArr, $flagArr);
                }
                $item->save(['flag' => implode(',', array_unique(array_filter($newflagArr)))]);
            }
            $this->success('设置成功');
        }
    }
    ...
    // 检查自定义URL是否存在
    public function checkcustomurlname()
    {
        $urlname = $this->request->param('urlname');
        // 添加时没有ID
        $id = $this->request->param('id');
        if (!$id) {
            $res = $this->model->where('custom_urlname', $urlname)->find();
        } else {
            $res = $this->model->where('custom_urlname', $urlname)->where('id', '<>', $id)->find();
        }

        if ($res) {
            $this->error('温馨提示：自定义URL已存在');
        }
        $this->success('');
    }
}
