    ...
    var Controller = {
        index: function () {
            ...
            $(document).on("click", ".btn-flag", function () {
                var ids = Table.api.selectedids(table);
                if (ids.length == 0) {
                    Toastr.error(__('Please select the data'));
                    return false;
                }
                Layer.open({
                    title: __('Set the batch identifier'),
                    ...
                    yes: function (index, layero) {
                        ...
                        Fast.api.ajax({
                            url: "ldcms/document/setflag/ids/" + ids.join(","),
                            type: "post",
        ...
        add: function () {
            ...
            Controller.api.events.checkCustomUrl($("form[role=form]"));
        },
        edit: function () {
            Controller.api.bindevent($("form[role=form]"));
            Controller.api.events.checkCustomUrl($("form[role=form]"));
        ...
        api: {
            events: {
                checkCustomUrl: function (form) {
                    var id = form.find('#c-id').val() || 0;
                    var si;
                    $(document).on("keyup", "#c-custom_urlname", function () {
                        var value = $(this).val();
                        ...
                        if (value != '' && !value.match(/\n/)) {
                            ...
                            si = setTimeout(function () {
                                Fast.api.ajax({
                                    loading: false,
                                    url: "ldcms/document/checkcustomurlname",
                                    data: { urlname: value, id: id },
                                }, function (data, ret) {
                                    return false;
                                });
                            }, 200);
