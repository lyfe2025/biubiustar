...
class Document extends Frontend
{
    ...
    public function info($table_name, $id)
    {
        ...
        // 获取扩展字段的text与list
        $this->appendTextAndList($result['documnent.mid'], $result);
        return $result;
    }
    ...
    public function getHomeList($params)
    {
        ...
        $listDb = $this::scope('status')->alias('document')
            ->join($join)
            ->field($field)
            ->where(function ($query) use ($cid, $mid, $id) {
                if (!empty($mid)) {
        ...
        // 新增：如果id不为空，按id顺序排序，否则按$order排序
        if (!empty($id)) {
            // 去除引号
            $id_field = trim($id, "'");
            $listDb = $listDb->orderRaw("FIELD(document.id, {$id_field})");
        } else {
        ...
        // 获取扩展字段的text与list
        $this->appendTextAndList($mid, $result, true);
    ...
    public function getHomeSearch($params)
    {
        ...
        $unfield = ['title','cid','mid','fuzzy','field','search','tags','lang','id','searchtpl','status','page','s','__searchtoken__','__token__','addon','controller','action','module','lg'];
        ...
        /*内容为空时，如果开启了搜索空内容时 返回404*/
        if ($result->isEmpty() && $addon_config['search_empty'] == '1') {
            abort('404', __('Not found'));
        }
    ...
    public function getHomeInfoById($id)
    {
        ...
        // 获取扩展字段的text与list
        $this->appendTextAndList($info['mid'], $info);
        return $info;
    }
     ...
     * @return mixed
     */
    public function appendTextAndList($mid, &$row, $isMultiArray = false)
    {
        $list = Fields::where('mid', $mid)
            ->field('id,field,type,content_list')
            ->where('status', 1)
            ->cache(true, null)
            ->select();
        $fieldsType = [];
        $fieldsList = [];
        $listFields = [
            'select', 'selects', 'checkbox', 'radio', 'array', 'selectpage', 'selectpages'
        ];
        foreach ($list as $field => $content) {
            $fieldsType[$content['field']] = $content['type'];
            if (in_array($content['type'], $listFields)) {
                $fieldsList[$content['field']] = $content['content_list_arr'];
            }
        }
